...
FORM SET_FIELDCAT CHANGING PT_FCAT TYPE LVC_T_FCAT.
  ...
  IF GT_ITEM IS NOT INITIAL.
    " 상단 ALV에서 PO No.를 클릭할 경우 GT_ITEM에 데이터가 SELECTION 되므로
    " [ GT_ITEM 데이터 존재 = 상단 ALV는 이미 생성됨 = 하단 ALV의 FCAT 생성 필요 ]를 의미함
...
*&---------------------------------------------------------------------< 251223 Cell 단위 편집을 위함(S)
    IF GV_EDIT EQ ABAP_ON.
      PERFORM CELL_EDIT_CONTROL USING 'MENGE' CL_GUI_ALV_GRID=>MC_STYLE_ENABLED
                             CHANGING GT_ITEM.
    ELSE.
      PERFORM CELL_EDIT_CONTROL USING 'MENGE' CL_GUI_ALV_GRID=>MC_STYLE_DISABLED
                             CHANGING GT_ITEM.
    ENDIF.
*&---------------------------------------------------------------------< 251223 Cell 단위 편집을 위함(E)
...
FORM INITIAL_VALUE .
  ...
  LV_YEAR = SY-DATUM+0(4) - 5.

  S_BEDAT-LOW = LV_YEAR && LV_MONTH && '01'.  " Low 값을 5년전 전월 1일로 설정
...
*&---------------------------------------------------------------------< 251223 : Cell 컨트롤 하기 위함
  GS_LAYO_ITEM-STYLEFNAME = 'CELLSTYL'.
*&---------------------------------------------------------------------<

ENDFORM.
*&---------------------------------------------------------------------*
*& Form CELL_EDIT_CONTROL
*&---------------------------------------------------------------------*
FORM CELL_EDIT_CONTROL USING VALUE(P_FNAME) VALUE(P_MC_STYLE)
                    CHANGING PT_ITEM TYPE TY_ITEM.

  LOOP AT PT_ITEM ASSIGNING FIELD-SYMBOL(<FS_ITEM>).

*    DATA(LS_ITEM) = VALUE LVC_S_STYL(
*                  FIELDNAME = P_FNAME
*                  STYLE = P_MC_STYLE
*          ).
*
*    INSERT LS_ITEM INTO TABLE <FS_ITEM>-CELLSTYL.
*    APPEND LS_ITEM TO <FS_ITEM>-CELLSTYL.

    INSERT VALUE LVC_S_STYL(
                  FIELDNAME = P_FNAME
                  STYLE = P_MC_STYLE
          ) INTO TABLE <FS_ITEM>-CELLSTYL.
